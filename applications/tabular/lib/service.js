/**
 * @fileOverview
 * A script to launch a Thywill server process running the Tabular example
 * application.
 */
  ...
  var config = {
    ...
    clientInterface: {
      ...
      elementAttributesHtml: {
        class: ['no-js'],
        lang: 'en',
        // The AngularJS application attribute, telling it to load the 'tabular'
        // module.
        'ng-app': 'tabular'
      },
...
exports.start = function () {
  ...
  if (typeof arguments[arguments.length - 1] === 'function') {
    ...
    callback = function (error, thywill) {
      if (error) {
        ...
        thywill.log.info('Thywill is ready to run the Tabular example application.');
      }
    };
  }
  ...
  // Add an Express route for the single Angular partial.
  app.get(config.clientInterface.baseClientPath + '/partial/main.html', function (req, res, next) {
    res.sendfile(path.join(__dirname, '../client/partial/main.html'), {
      maxAge: 0
    });
  });
  ...
  var tabular = new Tabular('tabular');

  // And off we go: launch a Thywill instance to run the the application.
  Thywill.launch(config, tabular, callback);
};
