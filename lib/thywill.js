...
/*
 * The expected section of the global configuration object pertaining to the Thywill class has the following format:
 * 
 * {
 *   "prepareForShutdown": {
 *     "port": 10080,
 ...
 * passedApplications - a single application or array of application objects to be registered.
 * callback - of the form function(error) {}, called on completion of setup, with error == null on success.
 */
p.startup = function(server, passedApplications, callback) {
  if( !this.config ) {
  ...
  this._initializeComponents(passedApplications, this.config, function(error) {
 ...
 * 
 * passedApplications - an array of objects of classes derived from Application
...
p._initializeComponents = function(passedApplications, config, callback) {
  ...
  // and finish by starting up the clientInterface, which will process all the resources defined so far
...
p._initializeComponent = function(componentType, config, callback) {
  ...
  // check the configuration: this will throw errors, unlike most of thywill's setup
