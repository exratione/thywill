...
p._startup = function(callback) {
  ...
  if( socketConfig ) {
    if( socketConfig.global && socketConfig.global instanceof Object ) {
      ...
      // we walk through the loop below applying whatever environment-specific Socket.IO configurations 
      // are provided in the configuration object.
      async.forEach(socketConfig.keys, function(environmentName) {
...
p._initializeConnection = function(socket) {
  ...
  // and emit this event on successful reconnectin.
  socket.on("reconnect", function() {
    self.reconnection(socket.client.id);
  });
};
