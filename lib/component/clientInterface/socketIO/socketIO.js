...
p._configure = function(config, callback) {
  ...
  var fns = [
    ...
    function(resources, asyncCallback) {
      ...
      var filepath = path.resolve(__dirname, "../../../client/thywill.html");
    ...
    function(asyncCallback) {
      var filepath = path.resolve(__dirname, "../../../client/thywill.js");
      var data = new String(fs.readFileSync(filepath));
      self.factory.resources.defineBootstrapResource(new Resource(Resource.TYPE_JAVASCRIPT, self.config.basePath + "/thywill.js", data), asycCallback);
    },
  ];
...
p._handleRequest = function(req, res) {
  ...
  this.factory.resources.load(requestData.pathname, function(error, resource) {
    if( error ) {
