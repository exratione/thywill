...
p._configure = function(config, callback) {
  ...
  this.serverListeners = this.factory.server.listeners("request");
  this.factory.server.removeAllListeners("request");
  this.factory.server.on("request", function (req, res) {
    ...
    function(resources, asyncCallback) {
      ...
      for( var i = 0, length = resources.length; i < length; i++ ) {
        if( resources[i].type == Resource.TYPE_JAVASCRIPT ) {
          js += self.factory.template.render(jsElementTemplate, { path: resources[i].path });
        } else if( resources[i].type == Resource.TYPE_HTML ) {
          css += self.factory.template.render(cssElementTemplate, { path: resources[i].path });
        }
      }
      data = self.factory.template.render(data, { js: js, css: css, encoding: self.config.encoding });
