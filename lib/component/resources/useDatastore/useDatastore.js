...
p.getBootstrapResources = function(callback) {
  var self = this;
  var fns = [];
  for( var i = 0, length = this.boostrapResourcePaths.length; i < length; i++ ) {
    fns.push(function(asynCallback) {
      self.getResource(this.boostrapResourcePaths[i], asyncCallback);
    });
  }
